...
| [`word-templates/`](word-templates/) | **Modèles de rapport Word** (`.docx`) pour le générateur de rapports à partir d'un modèle : des balises `{{ … }}` sont remplacées par les valeurs, tableaux, matrices et radars de l'analyse. Modèles *classique* et *éclaté par catégorie* (versions propre et annotée) + **catalogue des mots-clés**. Voir [`word-templates/README.md`](word-templates/README.md). |
