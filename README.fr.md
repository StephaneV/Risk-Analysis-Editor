...
### ▶️ [Ouvrir l'application](https://stephanev.github.io/Risk-Analysis-Editor/app/risk-analysis-editor.html) · 📖 [**Guide utilisateur**](docs/guide-utilisateur.md) · 🎬 [Tutoriels vidéo](https://www.youtube.com/@RiskAnalysisEditor) · ⬇️ [Télécharger](https://github.com/StephaneV/Risk-Analysis-Editor/releases/latest/download/risk-analysis-editor.html)

*Aucune installation : l'outil s'exécute entièrement dans votre navigateur. Deux démos prêtes à ouvrir : 📊 une [analyse de risques **inspirée d'EBIOS RM**](https://stephanev.github.io/Risk-Analysis-Editor/app/risk-analysis-editor.html?file=../examples/demo-ebios-rm-systeme-d-information-objets-enrichi.rae.json) (12 risques, 11 mesures) et 🛡️ [un exemple du volet d'analyse des risques d'une AIPD](https://stephanev.github.io/Risk-Analysis-Editor/app/risk-analysis-editor.html?file=../examples/demo-aipd-sst-objets-enrichi.rae.json), inspiré de la méthode PIA de la CNIL, centré sur les événements redoutés, les mesures et le risque résiduel (12 risques, 12 mesures) — toutes deux illustrant descriptions, notes, **tags** colorés, **barres de progression**, responsables et justifications de liens, ainsi qu'un **modèle d'objets réutilisables** (valeurs métier, biens supports, sources de risque, parties prenantes, scénarios…) et un rapport pré-paramétré. Le téléchargement fournit **l'unique fichier HTML** de la [dernière version](https://github.com/StephaneV/Risk-Analysis-Editor/releases/latest) : double-cliquez-le pour travailler **hors-ligne**.*
...
Toute l'analyse tient dans un fichier **`.rae.json`** autoportant : grille, risques, mesures, liens et cotations initiale/résiduelle. Le format est **spécifié** ([documentation technique](specs/SPEC-format-analyse-risque.md)) et accompagné d'un **schéma JSON** de validation ([schema-analyse-risque.json](specs/schema-analyse-risque.json)). Les noms de propriétés sont en anglais ; les valeurs (libellés, descriptions) restent dans la langue de l'analyse.

**Site web officiel :** [www.risk-analysis-editor.com](https://www.risk-analysis-editor.com/) — présentation, captures d'écran et liens.

**Chaîne YouTube :** [@RiskAnalysisEditor](https://www.youtube.com/@RiskAnalysisEditor) — tutoriels vidéo.
...
- **Plan d'action** : suivi des mesures via trois présentations (échéancier, kanban par statut, groupé par responsable), mesures **en retard** mises en évidence (échéance passée et non finalisée), et **avancement global** (barre + compteurs par statut). Chaque mesure se **modifie au double-clic** — ou via le **bouton ✎** de chaque ligne/carte — (échéance, statut, responsable, notes, champs personnalisés) et, dans le kanban, se **glisse-dépose** d'une colonne de statut à l'autre.
- **Champs personnalisés** : définissez dans *Paramètres* des champs supplémentaires rattachés à l'analyse, aux risques, aux mesures ou aux liens — de nombreux types (oui/non, entier, décimal, date, texte, texte long, URL, e-mail, téléphone, texte contrôlé par motif (regexp), **couleur**, **image** (embarquée, affichée en vignette avec loupe au clic), liste déroulante, liste à cocher, **tags colorés** en choix unique/multiple, **échelle** (niveaux à valeur numérique exploitables dans des calculs), **barre de progression** en %, et **valeur calculée** (formule style Excel recalculée en direct sur les autres champs et grandeurs dérivées), plus le type **référence** — voir ci-dessous), libellés multilingues (le libellé et l'aide se saisissent dans la langue de l'interface active ; à défaut de traduction, le code est affiché), caractère obligatoire et bornes (min/max, longueur, nombre d'items), et — pour les listes (déroulante, à cocher, tags) — une **description par valeur** réunie dans une section **« Référentiels »** du rapport ; les valeurs se saisissent dans les fiches (risques, mesures) et dans l'onglet *Présentation* (champs de l'analyse), avec validation, et sont reprises dans le **rapport** et dans l'**import/export CSV**.
- **Objets & références** : au-delà des simples champs personnalisés, définissez vos propres **types d'objets** (biens supports, valeurs métier, parties prenantes, sources de risque, destinataires des données…) dotés d'**attributs** typés, puis créez autant d'**instances** que nécessaire dans l'onglet dédié *Objets* (sous-onglets par type, tri, import/export CSV). Un champ **référence** relie ensuite un risque, une mesure, un lien, l'analyse ou un autre objet à une ou plusieurs instances — ou aux **risques/mesures** de l'analyse eux-mêmes, affichés en **pastilles R*x*/M*x*** cliquables (code + infobulle du libellé). Les références alimentent les filtres, les statistiques et les radars, apparaissent dans le rapport et les exports Word/Excel, et restent cohérentes grâce à la **suppression en cascade**. Un **visualiseur de schéma** (dans [`tools/`](tools/schema-viewer/)) dessine le modèle objets/références en diagramme entité-relation.
...
- **Vues des registres & colonnes** (Risques, Mesures, Détail des liens, Objets) : chaque registre propose trois présentations au choix via un **sélecteur d'icônes** — **Tableau**, **Maître·détail** (liste compacte + **tiroir de détail** par ligne) et **Cartes** (une fiche par élément) — plus un contrôle de **densité** (Confortable / Compact / Dense). Un **gestionnaire de colonnes** à trois états (bouton **⚙ Colonnes** dans la barre d'outils, présent dans toutes les vues) règle chaque colonne — y compris les **colonnes de champ personnalisé** — sur **En ligne** (colonne en Tableau et Maître·détail, affichée dans la fiche en Cartes), **En détail** (masquée du Tableau, reléguée au tiroir en Maître·détail, toujours dans la fiche en Cartes) ou **Masqué** (partout) ; par défaut, l'app classe automatiquement chaque colonne En ligne ou En détail (textes longs et champs secondaires partent En détail), jamais Masqué. On réordonne par les flèches ▲/▼ ou en glissant les en-têtes ; les colonnes ID et Actions restent épinglées ; une option **pleine largeur** et un menu **« Trier par ▾ »** (pour trier là où il n'y a pas d'en-tête, ex. Cartes) complètent l'ensemble. Les **notes des liens rendent le Markdown** dans les trois vues. **Édition :** un simple clic ne fait rien (un texte long tronqué s'agrandit, une image s'ouvre en visionneuse) ; un **double-clic** sur un champ ouvre la fiche positionnée sur ce champ, dans toutes les vues — avec un **bouton ✎** explicite dans le Plan d'action. Toute la disposition — vue, densité, états et ordre des colonnes — est **enregistrée dans le fichier** (`extensions.display`).
