...
- **Propagated filtering**: the dropdown filters for **category** (risk), **type** and **status** (measure), plus any **custom field** declared *usable as a filter*, **propagate along the links** — restricting every tab, the matrices and the report; criteria combine (AND) and each active propagated filter stays **visible and editable** in every bar where it applies. The propagated filter (base + custom fields) is **saved in the file** and restored when the analysis is reopened. Text search and the Action-plan-specific filters (*owner*, *overdue*) stay local to their view.
