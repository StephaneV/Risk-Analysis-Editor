...
- **Filtering by custom field**: a field with closed values (dropdown, checklist, tags, yes/no) can be flagged *usable as a filter*. It then appears as a filter list in every view showing the target object — Risks, Measures, Links, Matrices, Action plan — and the current filtering is reflected in the URL, so a filtered view can be shared by copying the address.
