...
L'onglet **Paramètres** comporte trois sous-onglets : *Affichage des matrices*, *Grille de cotation* et *Champs personnalisés*.
...
L'onglet **Rapport** génère un document complet, prêt à imprimer, qui reprend : le cartouche (métadonnées), un bloc **Présentation** (description et champs de l'analyse), la **synthèse** (compteurs et répartition par criticité), la **grille de cotation** détaillée, les **matrices** (Initial/Résiduel et Trajectoire) en vectoriel, le **registre des risques** et le **détail** des risques, les **mesures**, et les **liens**.
...
- **Filtrer.** Des filtres déroulants (catégorie, type, statut, responsable, « en retard uniquement », et tout champ personnalisé déclaré *filtrable*) restreignent l'affichage. Les filtres se **combinent (ET)** et se **propagent le long des liens** : filtrer sur un risque restreint aussi les mesures et les liens correspondants, et réciproquement. Un compteur « n sur N » et un bouton *Réinitialiser* apparaissent dès qu'un filtre ou une recherche est actif. Le filtrage courant est reflété dans l'**adresse (URL)**, ce qui permet de partager une vue filtrée.
...
- **Exporter en Word (.docx)** — reprend la structure du rapport (cartouche, présentation, synthèse, grille, **matrices en images**, registres et fiches détaillées). Prêt à fondre dans un gabarit d'entreprise. Également accessible via le bouton *⭳ Word* de l'onglet Rapport.
...
*Ce guide correspond à Risk Analysis Editor version 1.8.0. Les captures illustrent l'analyse de démonstration fictive `examples/demo-aipd-sst.rae.json`.*
